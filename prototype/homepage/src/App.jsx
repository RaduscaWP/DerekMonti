...
const LIVE = 'https://www.flywithderek.com';
...
const questions = [
  ...
  ['How do I contact Derek directly?', 'You can call, email or use WhatsApp through the links below. You will discuss your journey personally with Derek. This local website preview simulates the trip form and does not send him a request.'],
];
...
function HeroMedia({ reduced }) {
  ...
  useEffect(() => {
    ...
    if (!element) return;
    if (reduced || paused || !visible || !pageVisible || failed) { element.pause(); return; }
    let cancelled = false;
  ...
  return <div ref={host} className="hero-media">
    <img className="hero-poster" src="/assets/hero-window-video-poster.webp" width="1536" height="1024" alt="" fetchPriority="high" />
    {!reduced && <video ref={video} className={`hero-video${playing || paused ? ' is-visible' : ''}`} muted loop playsInline preload="metadata" poster="/assets/hero-window-video-poster.webp" aria-hidden="true" onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onError={() => setFailed(true)}>
      <source src="/assets/hero-window-loop.webm" type="video/webm" />
      <source src="/assets/hero-window-loop.mp4" type="video/mp4" onError={() => { setFailed(true); setPlaying(false); }} />
    </video>}
...
export function App() {
  ...
  function goToTrip() {
    ...
    requestAnimationFrame(() => {
      document.getElementById('trip-request').scrollIntoView({ behavior: reduced ? 'instant' : 'smooth' });
  ...
  return <div ref={page} data-motion={reduced ? 'reduced' : 'full'}>
    <a className="skip-link" href="#trip-request">Skip to trip planning</a>
    ...
    <dialog ref={menu} className="mobile-menu" aria-label="Main menu" onClose={() => { document.body.style.overflow = ''; }} onCancel={closeMenu}>
    ...
    <main id="top">
      <section className="hero" aria-labelledby="hero-title">
        ...
        <div className="hero-inner section-wrap">
          <div className="hero-copy">
            <p className="eyebrow hero-enter">Business & first class</p>
            ...
            <div className="hero-actions hero-enter"><button className="button button--primary" onClick={() => heroFrom.current?.focus()}>Plan my trip <ArrowRight size={21} /></button><a className="text-link" href="#meet-derek">Meet Derek</a></div>
            <div className="hero-advisor hero-enter"><img src="/assets/derek-avatar.jpg" width="88" height="88" alt="Derek Monti" /><div><strong>Derek Monti</strong><span>Your personal flight advisor.</span></div></div>
          ...
          <form className="route-entry hero-enter" onSubmit={continueRoute} aria-label="Start planning your trip">
            <div className="route-input"><PlaneTakeoff size={27} /><label htmlFor="hero-from"><span>From</span><input ref={heroFrom} id="hero-from" name="heroFrom" autoComplete="off" maxLength={80} value={heroRoute.from} onChange={(event) => updateHeroRoute('from', event.target.value)} placeholder="Where are you flying from?" required /></label></div>
            <div className="route-input"><PlaneLanding size={27} /><label htmlFor="hero-to"><span>To</span><input id="hero-to" name="heroTo" autoComplete="off" maxLength={80} value={heroRoute.to} onChange={(event) => updateHeroRoute('to', event.target.value)} placeholder="Where do you want to go?" required /></label></div>
            <button className="button button--primary route-submit" type="submit">Start planning <ArrowRight size={20} /></button>
      ...
      <section className="introduction section-wrap" id="meet-derek" aria-labelledby="intro-title">
        <div data-reveal><p className="eyebrow">A person behind your journey</p><h2 id="intro-title">The difference<br />is in the details.</h2></div>
        <div className="intro-copy"><p className="intro-lead">A good flight starts with someone who listens.</p><p>I'm Derek. I help you explore business and first class around your plans — the route, the cabin, the timing, and the little things that make travel feel easier.</p><p>You share what matters. I personally review the options with you.</p><a className="derek-intro-link" href={`${LIVE}/about`}><img src="/assets/derek-monti.jpg" width="52" height="52" alt="" loading="lazy" /><span>Get to know Derek<small>Your personal flight advisor</small></span><ArrowUpRight size={20} /></a></div>
      </section>

      <section id="comfort" className="comfort" aria-labelledby="comfort-title">
        <div className="comfort-inner section-wrap">
          <div className="comfort-copy"><p className="eyebrow">Your journey, your priorities</p><h2 id="comfort-title">How do you<br />want to arrive?</h2><p className="section-description">Start with what matters to you. Derek will use your priorities to review your trip.</p>
            <fieldset className="comfort-options"><legend className="sr-only">Choose your arrival priority, optional</legend>{COMFORT_OPTIONS.map((option, index) => <label key={option.value} className={trip.comfort === option.value ? 'is-selected' : ''}><span className="comfort-number">0{index + 1}</span><input type="radio" name="studio-priority" aria-label={option.label} value={option.value} checked={trip.comfort === option.value} onChange={() => setTrip((current) => ({ ...current, comfort: option.value }))} /><span className="comfort-option-copy"><strong>{option.label}</strong><span className={`comfort-explanation${shown.value === option.value ? ' is-shown' : ''}`}>{option.description}</span></span></label>)}</fieldset>
          ...
          <div className="comfort-visual" aria-label={visuals[shown.value]} role="img">{COMFORT_OPTIONS.map(({ value }) => <img key={value} className={shown.value === value ? 'is-active' : ''} src={`/assets/comfort-${value}.webp`} width="1200" height="1200" loading="lazy" decoding="async" alt="" aria-hidden="true" />)}<p>Illustrative cabin. Details vary by flight.</p></div>
        </div>
      </section>

      <section id="how-it-works" className="process section-wrap" aria-labelledby="process-title"><div className="section-heading" data-reveal><div><p className="eyebrow">Personally considered</p><h2 id="process-title">From your first thought<br />to your next departure.</h2></div><p>A simple conversation.<br />A journey shaped around you.</p></div><ol className="process-steps">{[
      ...
      <section className="request-section" id="trip-request" aria-labelledby="trip-section-title"><div className="section-wrap"><div className="request-heading"><p className="eyebrow">Your next chapter</p><h2 id="trip-section-title" tabIndex="-1">Where shall we go?</h2><p>Start your trip brief. Make it yours, one detail at a time.</p></div><TripForm trip={trip} setTrip={setTrip} step={step} setStep={setStep} /></div></section>

      <section className="faq section-wrap" aria-labelledby="faq-title"><div><p className="eyebrow">Before you take off</p><h2 id="faq-title">A few things<br />to know.</h2><a className="text-link text-link--dark" href={WHATSAPP} target="_blank" rel="noreferrer">Ask Derek a question <ArrowUpRight size={17} /></a></div><div className="faq-list">{questions.map(([question, answer], index) => <div className={`faq-item${openFaq === index ? ' is-open' : ''}`} key={question}><h3><button aria-expanded={openFaq === index} aria-controls={`faq-answer-${index}`} id={`faq-question-${index}`} onClick={() => setOpenFaq(openFaq === index ? null : index)}>{question}<ChevronDown size={19} /></button></h3><div id={`faq-answer-${index}`} role="region" aria-labelledby={`faq-question-${index}`} hidden={openFaq !== index}><p>{answer}</p></div></div>)}</div></section>

      <section className="closing" aria-labelledby="closing-title"><div className="section-wrap closing-inner"><div><p className="eyebrow">A higher way to see the world</p><h2 id="closing-title">Your journey.<br />My personal attention.</h2></div><div className="closing-actions"><button className="button button--white" onClick={goToTrip}>Plan my trip <ArrowRight size={20} /></button><a href={WHATSAPP} target="_blank" rel="noreferrer">Or start a conversation on WhatsApp <ArrowUpRight size={17} /></a></div></div></section>
    </main>

    <footer className="site-footer"><div className="section-wrap"><div className="footer-main"><div className="footer-brand"><Brand light /><p>Business & first class.<br />Personally arranged.</p></div><nav aria-label="Footer navigation"><span className="footer-label">Explore</span><a href="#comfort">The experience</a><a href={`${LIVE}/services`}>Services</a><a href={`${LIVE}/about`}>About Derek</a><a href={`${LIVE}/blog`}>Travel journal</a></nav><div className="footer-contact"><span className="footer-label">Let's talk travel</span><a href="tel:+17867064828">+1 (786) 706-4828</a><a href="mailto:Derek@travelbusinessclass.com">Derek@travelbusinessclass.com</a><a href={WHATSAPP} target="_blank" rel="noreferrer">WhatsApp <ArrowUpRight size={15} /></a></div></div><div className="footer-bottom"><p>© {new Date().getFullYear()} Fly with Derek</p><div><a href={`${LIVE}/privacy`}>Privacy</a><a href={`${LIVE}/terms`}>Terms</a><a href="#top">Back to top <ArrowUpRight size={14} /></a></div><label className="motion-preference"><input type="checkbox" checked={reduced} disabled={systemReduced} onChange={(event) => setQuietMotion(event.target.checked)} /> Reduce motion</label><p className="prototype-label">Local design preview</p></div></div></footer>
  </div>;
}
